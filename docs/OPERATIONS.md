...
| "Edit Answer" opens an empty dialog, or its button stays disabled | This was the stored `submissions.breakdown` lacking each question's `options`. Current builds re-attach the options from the question definitions, so it works without a migration; re-apply `homework-subpoints.sql` so newly graded papers store them too, then regrade the affected homework |
